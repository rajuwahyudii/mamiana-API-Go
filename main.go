...
func main() {
	...
	for {
		...
		if time == "20:00" {
			sendMessage("Selamat malam ibu..\nIngin mengingatkan sekarang sudah memasuki jam 20.00 waktunya ibu untuk meminum obat. Segera melakukan konfirmasi melalui aplikasi Si.X dengan mengupload obat yang akan diminum hari ini\nTerimakasih", nomorhp.(string))
			sendMessage("Tips : \n\n"+tips[strconv.Itoa(hari)].(string), nomorhp.(string))
		}
		// sendMessage("Selamat malam ibu..\nIngin mengingatkan sekarang sudah memasuki jam 20.00 waktunya ibu untuk meminum obat. Segera melakukan konfirmasi melalui aplikasi Si.X dengan mengupload obat yang akan diminum hari ini\nTerimakasih", nomorhp.(string))
		// sendMessage("Tips : \n\n"+tips[strconv.Itoa(hari)].(string), nomorhp.(string))
	}
